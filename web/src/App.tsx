...
import { api, type TelephonyView } from './api';
...
export function App() {
  ...
  const orders = useAsync<OrderView[]>(() => api.orders(), { pollMs: 15_000 });
      ...
      <header className="topbar">
        ...
        <div className="topbar-right">
          ...
          <button className="icon" title="Aggiorna" aria-label="Aggiorna"
            onClick={() => { orders.reload(); telephony.reload(); }}>↻</button>
        </div>
      </header>

      <main>
        {tab === 'orders' ? <OrdersPanel state={orders} /> : null}
